...
// Add this function to prevent all default button creation
function preventDefaultButtons() {
  console.log("Preventing default button creation...");
  
  // 1. Clear localStorage completely to remove any saved designs
  localStorage.clear();
  
  // 2. Override window.buttons.init to prevent default button creation
  const originalButtonsInit = window.buttons ? window.buttons.init : null;
  if (window.buttons && window.buttons.init) {
    window.buttons.init = function(pan, getImageMetadata, callbacks) {
      console.log("Overridden buttons.init - preventing default buttons");
      
      // Call the original init but don't create any default buttons
      if (originalButtonsInit) {
        originalButtonsInit.call(this, pan, getImageMetadata, callbacks);
      }
      
      // Clear any buttons that might have been created during init
      setTimeout(() => {
        const allButtons = this.getButtons ? this.getButtons() : [];
        if (allButtons.length > 0) {
          console.log("Cleaning up", allButtons.length, "default buttons");
          ...
          // Remove any button elements from DOM
  ...
  // 3. Override window.buttons.create to prevent creation without explicit call
  if (window.buttons && window.buttons.create) {
    const originalCreate = window.buttons.create;
    window.buttons.create = function(config) {
      // Only allow creation if config has a valid entityId (not a default)
      if (!config || !config.entityId || config.entityId.includes('default') || config.entityId === '') {
        console.log("Blocking default button creation:", config);
        return null;
      }
  ...
  // 4. Clear any existing buttons immediately
  setTimeout(() => {
    document.querySelectorAll(".light-button").forEach(btn => btn.remove());
    
    // Clear all module arrays
    if (window.buttons && window.buttons.getButtons) {
      window.buttons.getButtons().length = 0;
    }
    if (window.DimmerModule && window.DimmerModule.getDimmerButtons) {
      window.DimmerModule.getDimmerButtons().length = 0;
    }
    if (window.CCTModule && window.CCTModule.getCCTButtons) {
      window.CCTModule.getCCTButtons().length = 0;
    }
    if (window.RGBModule && window.RGBModule.getRGBButtons) {
      window.RGBModule.getRGBButtons().length = 0;
    }
    if (window.LockModule && window.LockModule.getLockButtons) {
      window.LockModule.getLockButtons().length = 0;
    }
    if (window.CurtainModule && window.CurtainModule.getCurtainButtons) {
      window.CurtainModule.getCurtainButtons().length = 0;
    }
    if (window.RemoteModule && window.RemoteModule.getRemoteButtons) {
      window.RemoteModule.getRemoteButtons().length = 0;
    }
    if (window.SensorModule && window.SensorModule.getSensorButtons) {
      window.SensorModule.getSensorButtons().length = 0;
    }
  }, 0);
  
  // 5. Modify the loadFromStorage function to not load default designs
  window.loadFromStorage = function() {
    try {
      const storedData = localStorage.getItem("floorplan_design_v2.0");
      if (storedData) {
        const designData = JSON.parse(storedData);
        
        // Check if this is a default/empty design
        if (designData.buttons && designData.buttons.length > 0) {
          // Check if it contains default buttons
          const hasDefaultButtons = designData.buttons.some(btn => 
            !btn.entityId || btn.entityId.includes('default') || btn.entityId === ''
          );
          
          if (hasDefaultButtons) {
            console.log("Skipping load of design with default buttons");
            localStorage.removeItem("floorplan_design_v2.0");
            return false;
          }
        }
        
        // Apply the design
        if (window.applyDesign) {
          window.applyDesign(designData);
        }
        return true;
      }
    } catch (error) {
  ...
  console.log("Default button prevention activated");
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // WebSocket configuration is loaded from config.js
  ...
  function updateButtonPositions() {
    if (window.buttons && window.buttons.updateButtonPositions) {
      window.buttons.updateButtonPositions()
    }
    if (window.DimmerModule && window.DimmerModule.updatePositions) {
      window.DimmerModule.updatePositions()
    }
    if (window.CCTModule && window.CCTModule.updatePositions) {
      window.CCTModule.updatePositions()
    }
    if (window.RGBModule && window.RGBModule.updatePositions) {
      window.RGBModule.updatePositions()
    }
    if (window.RemoteModule && window.RemoteModule.updatePositions) {
      window.RemoteModule.updatePositions()
    }
    if (window.LockModule && window.LockModule.updatePositions) {
      window.LockModule.updatePositions()
    }
    if (window.CurtainModule && window.CurtainModule.updatePositions) {
      window.CurtainModule.updatePositions()
    }
    if (window.SensorModule && window.SensorModule.updatePositions) {
      window.SensorModule.updatePositions()
    }
  }
  ...
  function saveDesign() {
    ...
    const allButtons = window.buttons ? window.buttons.getButtons() : []
    const dimmerButtons = window.DimmerModule ? window.DimmerModule.getDimmerButtons() : []
    const cctButtons = window.CCTModule ? window.CCTModule.getCCTButtons() : []
    const rgbButtons = window.RGBModule ? window.RGBModule.getRGBButtons() : []
    const lockButtons = window.LockModule ? window.LockModule.getLockButtons() : []
    const curtainButtons = window.CurtainModule ? window.CurtainModule.getCurtainButtons() : []
    const remoteButtons = window.RemoteModule ? window.RemoteModule.getRemoteButtons() : []
    const sensorButtons = window.SensorModule ? window.SensorModule.getSensorButtons() : []
      ...
      if (designData.buttons && Array.isArray(designData.buttons)) {
        ...
        designData.buttons.forEach((buttonConfig, index) => {
          ...
          const cleanConfig = {
            ...
            sensorConfig: buttonConfig.sensorConfig || buttonConfig.stateConfigurations || [],

            currentState: buttonConfig.currentState || "unknown",
          }
          ...
          if (cleanConfig.type === "dimmer") {
            if (window.DimmerModule && window.DimmerModule.create) {
              window.DimmerModule.create(cleanConfig)
            }
          } else if (cleanConfig.type === "cct") {
            if (window.CCTModule && window.CCTModule.create) {
              window.CCTModule.create(cleanConfig)
            }
          } else if (cleanConfig.type === "rgb") {
            if (window.RGBModule && window.RGBModule.create) {
              window.RGBModule.create(cleanConfig)
            }
          } else if (cleanConfig.type === "lock") {
            if (window.LockModule && window.LockModule.create) {
              window.LockModule.create(cleanConfig)
            }
          } else if (cleanConfig.type === "curtain") {
            if (window.CurtainModule && window.CurtainModule.create) {
              window.CurtainModule.create({
                ...cleanConfig,
                currentPosition: typeof buttonConfig.currentPosition === "number" ? buttonConfig.currentPosition : 0,
                isOpen: typeof buttonConfig.currentPosition === "number" ? buttonConfig.currentPosition > 0 : false,
              })
            }
          } else if (cleanConfig.type === "remote") {
            if (window.RemoteModule && window.RemoteModule.create) {
          ...
          } else if (cleanConfig.type === "sensor" && window.SensorModule && window.SensorModule.create) {
            window.SensorModule.create({
              ...cleanConfig,
              sensorConfig: cleanConfig.sensorConfig || cleanConfig.stateConfigurations,
            })
          } else {
            if (window.buttons && window.buttons.create) {
              window.buttons.create(cleanConfig)
            }
          }
        })
      }
  ...
  function clearAllButtons() {
    ...
    allButtons.forEach((button) => {
      if (window.buttons) {
        window.buttons.deleteButton(button.id)
      }
    })

    if (window.DimmerModule && window.DimmerModule.getDimmerButtons) {
      const allDimmers = window.DimmerModule.getDimmerButtons()
      allDimmers.forEach((dimmer) => {
        window.DimmerModule.deleteButton(dimmer.id)
      })
    }

    if (window.CCTModule && window.CCTModule.getCCTButtons) {
      const allCCTs = window.CCTModule.getCCTButtons()
      allCCTs.forEach((cct) => {
        window.CCTModule.deleteButton(cct.id)
      })
    }

    if (window.RGBModule && window.RGBModule.getRGBButtons) {
      const allRGBs = window.RGBModule.getRGBButtons()
      allRGBs.forEach((rgb) => {
        window.RGBModule.deleteButton(rgb.id)
      })
    }

    if (window.LockModule && window.LockModule.getLockButtons) {
      const allLocks = window.LockModule.getLockButtons()
      allLocks.forEach((lock) => {
        window.LockModule.deleteButton(lock.id)
      })
    }

    if (window.CurtainModule && window.CurtainModule.getCurtainButtons) {
      const allCurtains = window.CurtainModule.getCurtainButtons()
      allCurtains.forEach((curtain) => {
        window.CurtainModule.deleteButton(curtain.id)
      })
    }

    if (window.RemoteModule && window.RemoteModule.getRemoteButtons) {
      const allRemotes = window.RemoteModule.getRemoteButtons()
      allRemotes.forEach((remote) => {
        window.RemoteModule.deleteButton(remote.id)
      })
    }

    if (window.SensorModule) {
      const sensorButtons = window.SensorModule.getSensorButtons()
      sensorButtons.forEach((sensor) => {
        window.SensorModule.deleteButton(sensor.id)
      })
    }
  }
...
`
    ...
    if (newLoadBtn) {
      newLoadBtn.addEventListener("click", () => {
        loadFileInput.click()  // Opens file picker
      })
    }
  }
  ...
  function setupEventListeners() {
    ...
    editBtn.addEventListener("click", () => {
      ...
      if (isEditMode) {
        ...
        if (buttonOpacity) {
          const opacityVal = Number.parseFloat(buttonOpacity.value)
          applyButtonOpacity(opacityVal)
        }

        if (window.buttons) window.buttons.enableEditMode(true)
        if (window.DimmerModule && window.DimmerModule.enableEditMode) {
          window.DimmerModule.enableEditMode(true)
        }
        if (window.CCTModule && window.CCTModule.enableEditMode) {
          window.CCTModule.enableEditMode(true)
        }
        if (window.RGBModule && window.RGBModule.enableEditMode) {
          window.RGBModule.enableEditMode(true)
        }
        if (window.LockModule && window.LockModule.enableEditMode) {
          window.LockModule.enableEditMode(true)
        }
        if (window.CurtainModule && window.CurtainModule.enableEditMode) {
          window.CurtainModule.enableEditMode(true)
        }
        if (window.RemoteModule && window.RemoteModule.enableEditMode) {
          window.RemoteModule.enableEditMode(true)
        }
        ...
        if (window.buttons) window.buttons.enableEditMode(false)
        if (window.DimmerModule && window.DimmerModule.enableEditMode) {
          window.DimmerModule.enableEditMode(false)
        }
        if (window.CCTModule && window.CCTModule.enableEditMode) {
          window.CCTModule.enableEditMode(false)
        }
        if (window.RGBModule && window.RGBModule.enableEditMode) {
          window.RGBModule.enableEditMode(false)
        }
        if (window.LockModule && window.LockModule.enableEditMode) {
          window.LockModule.enableEditMode(false)
        }
        if (window.CurtainModule && window.CurtainModule.enableEditMode) {
          window.CurtainModule.enableEditMode(false)
        }
        if (window.RemoteModule && window.RemoteModule.enableEditMode) {
          window.RemoteModule.enableEditMode(false)
        }
        if (window.SensorModule && window.SensorModule.enableEditMode) {
  ...
  document.getElementById("buttonEditForm").addEventListener("submit", (e) => {
    ...
    if (btnType === "dimmer" && window.DimmerModule) {
      ...
      const success = window.DimmerModule.updateConfig(btnId, {
        entityId: entityId,
        name: name || "Dimmer",
        iconClass: icon,
      })
      ...
      const success = window.CCTModule.updateConfig(btnId, {
        entityId: entityId,
        name: name || "CCT Light",
        iconClass: icon,
      })
      ...
      const success = window.RGBModule.updateConfig(btnId, {
        entityId: entityId,
        name: name || "RGB Light",
        iconClass: icon,
      })
      ...
      const success = window.LockModule.updateConfig(btnId, {
        entityId: entityId,
        name: name || "Lock",
        iconClass: icon,
      })
      ...
      const success = window.CurtainModule.updateConfig(btnId, {
        entityId: entityId,
        name: name || "Curtain",
        iconClass: icon,
      })
      ...
      const success = window.RemoteModule.updateConfig(btnId, {
        entityId: entityId,
        name: name || "Remote",
        iconClass: icon,
      })
      console.log("Remote update result:", success)
      // Add sensor update handling
    } else if (btnType === "sensor" && window.SensorModule) {
      console.log("Updating sensor button:", btnId)
      const success = window.SensorModule.updateConfig(btnId, {
        entityId: entityId,
        name: name || "Sensor",
        iconClass: icon,
      })
      ...
      const success = window.buttons.updateButtonConfig(btnId, {
        entityId: entityId,
        name: name || "Button",
        iconClass: icon,
      })
  ...
  function updateAllButtonsFromState(states) {
    ...
    window.buttons.getButtons().forEach((button) => {
      ...
      if (st) {
        ...
        if (domain === "light" || domain === "fan") {
          ...
          if (brightness !== undefined) {
            ...
            if (window.DimmerModule && window.DimmerModule.handleStateUpdate) {
              window.DimmerModule.handleStateUpdate(button.entityId, st.state, brightnessPercent)
            }

            // Update CCT module if color temp exists
            if (st.attributes?.color_temp) {
              if (window.CCTModule && window.CCTModule.handleStateUpdate) {
                window.CCTModule.handleStateUpdate(
                  button.entityId,
                  st.state,
                  brightnessPercent,
            ...
            if (st.attributes?.hs_color) {
              if (window.RGBModule && window.RGBModule.handleStateUpdate) {
                window.RGBModule.handleStateUpdate(button.entityId, st.state, brightnessPercent, st.attributes.hs_color)
              }
            }
          }
        }
    ...
    if (window.DimmerModule && window.DimmerModule.getDimmerButtons) {
      window.DimmerModule.getDimmerButtons().forEach((dimmer) => {
        ...
        if (st) {
          ...
          window.DimmerModule.handleStateUpdate(dimmer.entityId, st.state, brightnessPercent)
        }
      })
    }
    ...
    // Update CCT buttons
    if (window.CCTModule && window.CCTModule.getCCTButtons) {
      window.CCTModule.getCCTButtons().forEach((cct) => {
        const st = states.find((s) => s.entity_id === cct.entityId)
        if (st) {
          const brightness = st.attributes?.brightness
          const brightnessPercent = brightness ? Math.round((brightness / 255) * 100) : 0
          const colorTemp = st.attributes?.color_temp
          window.CCTModule.handleStateUpdate(cct.entityId, st.state, brightnessPercent, colorTemp)
        }
      })
    }
    ...
    // Update RGB buttons
    if (window.RGBModule && window.RGBModule.getRGBButtons) {
      window.RGBModule.getRGBButtons().forEach((rgb) => {
        const st = states.find((s) => s.entity_id === rgb.entityId)
        if (st) {
          const brightness = st.attributes?.brightness
          const brightnessPercent = brightness ? Math.round((brightness / 255) * 100) : 0
          const hsColor = st.attributes?.hs_color
          window.RGBModule.handleStateUpdate(rgb.entityId, st.state, brightnessPercent, hsColor)
        }
      })
    }
    ...
    // Update lock buttons
    if (window.LockModule && window.LockModule.getLockButtons) {
      window.LockModule.getLockButtons().forEach((lock) => {
        const st = states.find((s) => s.entity_id === lock.entityId)
        if (st) {
          const isLocked = st.state === "locked"
          window.LockModule.handleStateUpdate(lock.entityId, st.state)
        }
      })
    }
    ...
    if (window.CurtainModule && window.CurtainModule.getCurtainButtons) {
      window.CurtainModule.getCurtainButtons().forEach((curtain) => {
        ...
        if (st) {
          ...
          window.CurtainModule.handleStateUpdate(curtain.entityId, st.state, position)
        }
      })
    }
    ...
    if (window.RemoteModule && window.RemoteModule.getRemoteButtons) {
      window.RemoteModule.getRemoteButtons().forEach((remote) => {
        const st = states.find((s) => s.entity_id === remote.entityId)
        if (st) {
          window.RemoteModule.handleStateUpdate(remote.entityId, st.state, st.attributes)
        }
      })
    }

    // Update sensor buttons
    if (window.SensorModule && window.SensorModule.getSensorButtons) {
      console.log("[v0] Updating sensor buttons from states")
      window.SensorModule.getSensorButtons().forEach((sensor) => {
        const st = states.find((s) => s.entity_id === sensor.entityId)
        if (st) {
          console.log("[v0] Found state for sensor:", sensor.entityId, st.state)
          window.SensorModule.handleStateUpdate(sensor.entityId, st.state)
        } else {
          console.log("[v0] No state found for sensor:", sensor.entityId)
        }
      })
    }
  }
  ...
  function updateButtonFromStateChange(entityId, newState) {
    const allLights = window.buttons.getButtons().filter((l) => l.entityId === entityId)
    allLights.forEach((light) => {
      ...
      if (light.type === "dimmer") {
        ...
        const brightnessPercent = brightness ? Math.round((brightness / 255) * 100) : 0
        if (window.DimmerModule && window.DimmerModule.handleStateUpdate) {
          window.DimmerModule.handleStateUpdate(entityId, newState.state, brightnessPercent)
        }
        ...
        if (window.CCTModule && window.CCTModule.handleStateUpdate) {
          window.CCTModule.handleStateUpdate(entityId, newState.state, brightnessPercent, colorTemp)
        }
        ...
        if (window.RGBModule && window.RGBModule.handleStateUpdate) {
          window.RGBModule.handleStateUpdate(entityId, newState.state, brightnessPercent, hsColor)
        }
      }
    })
    ...
    if (window.LockModule && window.LockModule.handleStateUpdate) {
      window.LockModule.handleStateUpdate(entityId, newState.state)
    }

    // Handle curtain state updates
    if (window.CurtainModule && window.CurtainModule.handleStateUpdate) {
      const position = newState.attributes?.current_position || newState.attributes?.position || 50
      window.CurtainModule.handleStateUpdate(entityId, newState.state, position)
    }

    // Handle remote state updates
    if (window.RemoteModule && window.RemoteModule.handleStateUpdate) {
      window.RemoteModule.handleStateUpdate(entityId, newState.state, newState.attributes)
    }

    // Handle sensor state updates
    if (window.SensorModule && window.SensorModule.handleStateUpdate) {
      console.log("[v0] Sensor state change detected:", entityId, newState.state)
      window.SensorModule.handleStateUpdate(entityId, newState.state)
    }

    // Update sensor buttons via EntityButtons
    if (window.EntityButtons && window.EntityButtons[entityId]) {
      console.log("[v0] Updating via EntityButtons:", entityId)
      window.EntityButtons[entityId].forEach((entityButton) => {
        if (entityButton.type === "sensor" && entityButton.handleStateUpdate) {
          console.log("[v0] Calling entityButton.handleStateUpdate")
          entityButton.handleStateUpdate(newState.state)
        }
      })
    }
  }
  ...
  function disableAllButtons() {
    window.buttons.getButtons().forEach((light) => {
  ...
  function toggleLight(entityId, buttonId) {
    ...
    const light = window.buttons.getButtons().find((l) => l.entityId === entityId)
    if (!light) return
    ...
    if (light.type === "dimmer") {
      if (window.DimmerModule && window.DimmerModule.openDimmerModal) {
        window.DimmerModule.openDimmerModal(light)
      }
      return
    } else if (light.type === "cct") {
      if (window.CCTModule && window.CCTModule.openCCTModal) {
        window.CCTModule.openCCTModal(light)
      }
      return
    } else if (light.type === "rgb") {
      if (window.RGBModule && window.RGBModule.openRGBModal) {
        window.RGBModule.openRGBModal(light)
      }
      return
    } else if (light.type === "remote") {
      // Open remote modal instead of toggling
      if (window.RemoteModule && window.RemoteModule.openRemoteModal) {
        window.RemoteModule.openRemoteModal(light)
      }
      return
    } else if (light.type === "sensor") {
      // Open sensor modal if available
      if (window.SensorModule && window.SensorModule.openSensorModal) {
        window.SensorModule.openSensorModal(light)
      }
      return
    }
  ...
  function getButtonType(buttonId) {
    ...
    if (btn.classList.contains("sensor")) return "sensor" // Add sensor type
    return "toggle"
  }
  ...
  function updateLightUI(buttonId, isOn) {
    ...
    const light = window.buttons.getButtons().find((l) => l.id === buttonId)
  ...
  function init() {
    ...
    if (window.buttons) {
      window.buttons.init(pan, getImageMetadata, {
        toggleLight: toggleLight,
        updateBrightness: updateBrightness,
      })
    }

    let DimmerModule, CCTModule, RGBModule, LockModule, CurtainModule, RemoteModule, SensorModule
    ...
    if (window.SensorModule && typeof window.SensorModule.init === "function") {
      console.log("[v0] Initializing SensorModule...")
      SensorModule = window.SensorModule.init(pan, getImageMetadata, {
        getImageMeta: getImageMetadata,
        send: (domain, service, data) => {
          if (!ready || !ws || ws.readyState !== WebSocket.OPEN) {
            console.warn("WS not ready")
            return
          }
      ...
      if (SensorModule) {
        window.SensorModule = SensorModule
        console.log("[v0] SensorModule API exposed:", Object.keys(SensorModule))
      }
    } else {
      console.warn("[v0] SensorModule not found on window")
    }
    ...
    // Load design - Only from localStorage (no default file)
    if (!loadFromStorage()) {
      // Don't auto-load default file
      img.src = "image.png"
      updateFooter("Ready - Click Load to import a design")
    }
