...
const productData = {
    "DOOR LOCK": {
        ...
        title: "Equipments",
        desc: "Lighting and fan equipment with CDN icons",
        isEquipment: true,
        subProducts: {
            "LIGHT": {
                title: "Light",
...
function createAccessPointConfigurationControls() {
    ...
    <div class="mark-controls-box" id="accessPointConfigControls" style="margin-top: 20px; border-color: #2196F3;">
        ...
        ${selectedMarkId ? `
        <div class="useless" style="background: #E3F2FD; padding: 12px; border-radius: 6px; margin-bottom: 15px; border: 1px solid #BBDEFB;">
...
if (data.isDBBox) {
    ...
    if (isSwitchFamily) {
        ...
        if (currentProduct === 'ACCESS POINT') {
            accessPointConfig = productData[currentProduct].defaultAccessPointConfig || '';
            if (accessPointConfig) {
                // For access points, configuration IS the model
                modelName = accessPointConfig;
            }
        } else {
            switchConfig = productData[currentProduct].defaultSwitchConfig || '';
            if (switchConfig) {
                // For switches, append configuration
                modelName = `${modelName} (${switchConfig})`;
            }
        }
...
document.getElementById('downloadPdfBtn').addEventListener('click', async function () {
    try {
        ...
        if (hasMainProducts) {
            ...
            // Create table data with configuration
            const mainTableData = tables.mainProducts.map(item => {
                // For switches and access points, the model field already contains configuration
                return [
                    item.label,
                    item.category,
                    item.brand,
                    item.model, // This already has "Model (Configuration)" or just configuration for AP
                    item.quantity.toString()
                ];
            });
            ...
            doc.autoTable({
                startY: yPosition,
                head: [['Label', 'Category', 'Brand', 'Model/Configuration', 'Qty']],
                body: mainTableData,
                theme: 'grid',
                headStyles: {
                    fillColor: [76, 175, 80],
                ...
                columnStyles: {
                    ...
                    2: { cellWidth: 30, halign: 'center' },
                    3: { cellWidth: 80, halign: 'left' }, // Wider column for model/configuration
...
function generateProductTable() {
    ...
    marks.forEach(mark => {
        ...
        // KEEP DB BOX CODE AS IS - DON'T CHANGE
        if (mark.isDBBox) {
            dbBoxesTable.push({
                label: mark.seriesLabel,
                category: category,
            ...
            return; // Return early - don't process DB boxes further
        }

        // KEEP NETWORK DB BOX CODE AS IS - DON'T CHANGE
        if (mark.isNetworkDBBox) {
            ...
            return; // Return early - don't process Network DB boxes further
        }

        // NOW HANDLE SWITCHES AND ACCESS POINTS
        if (mark.isSwitchFamily) {
            if (mark.categoryName === 'ACCESS POINT') {
                // For access points, use the configuration as the model
                const apConfig = mark.accessPointConfig || productData[mark.categoryName]?.defaultAccessPointConfig || '';
                model = apConfig || 'Access Point';
                if (model === 'Access Point') {
                ...
                if (switchConfig) {
                    ...
                    model = `${baseModel} (No configuration)`;
                }
            }
        }
        ...
        // Handle regular products (including switches/access points)
        if (!productMap.has(key)) {
            productMap.set(key, {
                label: key,
                category: category,
                brand: brand,
                model: model, // This now includes configuration for switches/APs
                quantity: 1
            });
        } else {
    ...
    return {
        ...
        dbBoxes: dbBoxesTable,  // This stays exactly as you have it
        networkDBBoxes: networkDBBoxesTable,  // This stays exactly as you have it
